...
static juce::ValueTree tuningTableMapToValueTree(const Keytographer::TuningTableMap& midiMap)
{
    ...
    //auto pattern = juce::ValueTree(Multimapper::ID::Pattern);
    //for (auto value : definition.map->pattern())
    //{
    //    auto node = juce::ValueTree(Multimapper::ID::Pattern);
    //    node.setProperty(Multimapper::ID::Value, value, nullptr);
    //    pattern.addChild
    //}
